...
using NewProject.Services.Entities.CustomAttributes;
using NPOI.HSSF.UserModel;
using NPOI.XSSF.UserModel;
using NewProject.Services.Entities.Common;

namespace NewProject.API.Infrastructure.Helper
{
    public class ExportImportHelper
    {
          
        public static void WriteData<T>(List<T> exportData, IWorkbook _workbook, ISheet _sheet, bool isExcel)
        {
        ...
        public static DataTable GetDataTable(IFormFile file, PaginationDto request)
        {
            ISheet sheet;
            var fileExt = Path.GetExtension(file.FileName);
            if (fileExt == ".xls")
            ...
            lastRow = request.PageIndex != null ? request.PageIndex.Value * request.PageSize.Value : lastRow;
            int startRowNumber = request.PageIndex != null ? lastRow - request.PageSize.Value + 1 : sheet.FirstRowNum + 1;

            for (int i = startRowNumber; i <= lastRow; i++)
            {
                var row = sheet.GetRow(i);
                if (row != null)
                {
